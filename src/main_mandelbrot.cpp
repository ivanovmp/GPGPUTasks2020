...
//    float sizeX = 2.0f;
    ...
    images::Image<unsigned char> cpu_image(width, height, 3);
...

    {
        ...
        for (int i = 0; i < benchmarkingIters; ++i) {
            mandelbrotCPU(cpu_results.ptr(),
                          ...
                          sizeX, sizeY,
                          iterationsLimit, performSmoothing);
            t.nextLap();
        }
        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = area * iterationsLimit * flopsInLoop;
        ...
        renderToColor(cpu_results.ptr(), cpu_image.ptr(), width, height);
        cpu_image.savePNG("mandelbrot_cpu.png");
    }
    ...
    context.activate();
    {
        ...
        unsigned int workGroupSize_0 = 16;
        unsigned int workGroupSize_1 = 8;
        gpu::gpu_mem_32f gpu_mem_results;
        gpu_mem_results.resizeN(area);
        unsigned int global_work_size_0 = (width + workGroupSize_0 - 1) / workGroupSize_0 * workGroupSize_0;
        unsigned int global_work_size_1 = (height + workGroupSize_1 - 1) / workGroupSize_1 * workGroupSize_1;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize_0, workGroupSize_1, global_work_size_0, global_work_size_1),
                gpu_mem_results,
                width, height,
                centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                sizeX, sizeY,
                iterationsLimit, (int) performSmoothing);
            t.nextLap();
        }
        gpu_mem_results.readN(gpu_results.ptr(), area);
